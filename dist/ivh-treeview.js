...
angular.module('ivh.treeview').directive('ivhTreeviewSelect', [function() {
  ...
  return {
    ...
    link: function(scope, element, attrs, trvw) {
      ...
      element.addClass('ivh-treeview-select');
...
angular.module('ivh.treeview').directive('ivhTreeviewInsert', [function() {
  'use strict';
  return {
    restrict: 'A',
    require: '^ivhTreeview',
    link: function(scope, element, attrs, trvw) {
      ...
      element.addClass('ivh-treeview-insert');
...
angular.module('ivh.treeview').directive('ivhTreeviewDelete', [function() {
  'use strict';
  return {
    restrict: 'A',
    require: '^ivhTreeview',
    link: function(scope, element, attrs, trvw) {
      ...
      element.addClass('ivh-treeview-delete');
...
angular.module('ivh.treeview').directive('ivhTreeview', ['ivhTreeviewMgr', function(ivhTreeviewMgr) {
  ...
  return {
    ...
    scope: {
      ...
      onSelect: '&ivhTreeviewOnSelect',
      onInsert: '&ivhTreeviewOnInsert',
      onDelete: '&ivhTreeviewOnDelete',
    ...
    controller: ['$scope', '$element', '$attrs', '$transclude', 'ivhTreeviewOptions', 'filterFilter', function($scope, $element, $attrs, $transclude, ivhTreeviewOptions, filterFilter) {
      ...
      /**
       * Call the registered select handler
       *
       * Handler will get a reference to `node` and the root of the tree.
       *
       * @param {Object} node Tree node to pass to the handler
      ...
      /**
       * Call the registered insert handler
       *
       * Handler will get a reference to `node` and the root of the tree.
       *
       * @param {Object} node Tree node to pass to the handler
       * @private
       */
      trvw.onInsert= function(node) {
        if(localOpts.onInsert) {
          var locals = {
            ivhNode: node,
            ivhIsExpanded: trvw.isExpanded(node),
            ivhTree: $scope.root
          };
          localOpts.onInsert(locals);
        }
      };
       ...
       * @private
       */
      trvw.onDelete= function(node) {
        if(localOpts.onDelete) {
          var locals = {
            ivhNode: node,
            ivhIsExpanded: trvw.isExpanded(node),
            ivhTree: $scope.root
          };
          localOpts.onDelete(locals);
        }
      };
